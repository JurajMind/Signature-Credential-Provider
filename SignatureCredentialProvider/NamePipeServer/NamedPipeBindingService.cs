...
namespace WpfWcfNamedPipeBinding
{
    ...
    {
        private static readonly Dictionary<INamedPipeBindingCallbackService, string> _clientCallBackService =
            new Dictionary<INamedPipeBindingCallbackService, string>();
        ...
        public void MessageToSomeone(CommunicatUnit composite, string someoneName)
        {
            if (_clientCallBackService.Values.Contains(someoneName))
            {
                ...
                KeyValuePair<INamedPipeBindingCallbackService, string> serve =
                    _clientCallBackService.Where(p => p.Value == someoneName).First();
                var cu = new CommunicatUnit();
        ...
        {
            var callback = OperationContext.Current.GetCallbackChannel<INamedPipeBindingCallbackService>();
