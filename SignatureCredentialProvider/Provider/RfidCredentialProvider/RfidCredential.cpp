	...
	)
{
	...
	if (SUCCEEDED(hr))
	{
		hr = SHStrDupW(L"Signature", &_rgFieldStrings[SFI_TITLE]);
	}
	if (SUCCEEDED(hr))
	{
		if(!username){
			hr = SHStrDupW(L"Sign in form to log in", &_rgFieldStrings[SFI_DETAILS]);
	...
	{
		if(!username){
			hr = SHStrDupW(L"Sign the form", &_rgFieldStrings[SFI_MINIDETAILS]);
		}else{
...
HRESULT RfidCredential::SetSelected(BOOL* pbAutoLogon)  
{
	...
	//system("start \"C:\\Users\\Juraj\\Desktop\\client\\Client1.exe\" &");
